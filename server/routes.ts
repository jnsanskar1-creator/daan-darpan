...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/database/backup", isAdminOrOperator, async (req, res) => {
    try {
      ...
      const emailOptions = {
        ...
        html: `
          ...
          <ul>
            ...
            <li><strong>Time:</strong> ${new Date().toLocaleTimeString('en-IN')}</li>
            <li><strong>Filename:</strong> ${filename}</li>
          </ul>
          
          <p>Please keep this file safe. It contains all user data, entries, and financial records.</p>
      ...
      const emailPromises = operatorResult.rows.map((operator: any) => {
  ...
  // Database Restore API - Admin Only
  app.post("/api/database/restore", isAdmin, upload.single('file'), async (req, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ message: "No file uploaded" });
      }
      ...
      const sheetToTable: Record<string, string> = {
        "Users": "users",
        "Entries": "entries",
        "Dravya_Entries": "dravya_entries",
        "Expense_Entries": "expense_entries",
        "Advance_Payments": "advance_payments",
        "Advance_Payment_Usage": "advance_payment_usage",
        "Boli_Payments": "boli_payments",
        "Previous_Outstanding_Records": "previous_outstanding_records",
      ...
      // Delete in reverse order of foreign key dependencies (children first)
      await query(`DELETE FROM transaction_logs`);
      await query(`DELETE FROM advance_payment_usage`);
      await query(`DELETE FROM boli_payments`);
      await query(`DELETE FROM previous_outstanding_records`);
      await query(`DELETE FROM advance_payments`);
      await query(`DELETE FROM expense_entries`);
      await query(`DELETE FROM dravya_entries`);
      ...
      for (const sheetName of restoreOrder) {
        ...
        if (!workbook.SheetNames.includes(sheetName)) {
          console.log(`Sheet '${sheetName}' not found in backup, skipping.`);
          continue;
        }
        ...
        stats[tableName] = { total: data.length, inserted: 0, errors: 0 };
      ...
      try {
        const sequences = [
          'users_id_seq', 'entries_id_seq', 'dravya_entries_id_seq',
          'expense_entries_id_seq', 'advance_payments_id_seq', 'advance_payment_usage_id_seq',
          'boli_payments_id_seq', 'previous_outstanding_records_id_seq', 'transaction_logs_id_seq',
          'corpus_settings_id_seq'
        ];

        for (const seq of sequences) {
          const table = seq.replace('_id_seq', '');
          await query(`SELECT setval('${seq}', (SELECT COALESCE(MAX(id), 0) + 1 FROM ${table}), false)`);
        }
      } catch (seqErr) {
